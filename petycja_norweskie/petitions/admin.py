...
from django.contrib.admin import FieldListFilter, RelatedOnlyFieldListFilter
...
class PetitionAdmin(admin.ModelAdmin):
    list_display = ('title', 'campaign', 'ask_first_name', 'ask_second_name', 'ask_organization', 'ask_city',
    ...
    def get_queryset(self, request):
        return super(PetitionAdmin, self).get_queryset(request).annotate(signature_count=Count('signature__id', distinct=True),
                                                                         definition_count=Count('permissiondefinition__id', distinct=True))
...
class AuthenticatedListFilter(RelatedOnlyFieldListFilter):
    def queryset(self, request, queryset):
        return super().queryset(request, queryset).for_admin(request.user)
...
class SignatureAdmin(ExportMixin, admin.ModelAdmin):
    ...
    list_filter = (
        ('petition', AuthenticatedListFilter),
        ('petition__campaign', AuthenticatedListFilter),
    )
